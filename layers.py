...
class NormalCalculator(nn.Module):
    ...
    def forward_from_depth(self, depth, K):
        ...
        """
        B, _, H, W = depth.shape
        if not hasattr(self, 'backproject'):
            self.backproject = BackprojectDepth(self.batch_size, self.height, self.width)
    ...
    def forward_from_points(self, points_3d):
        ...
        if points_3d.dim() == 3:
            points_reshaped = points_3d.view(B, points_3d.shape[1], H, W)
            xyz = points_reshaped[:, :3, :, :]
        else:
            xyz = points_3d[:, :3, :, :]
        ...
        neigh = torch.stack(neigh_list, dim=4)
        
        vec = neigh - xyz.unsqueeze(4)
        vec1 = vec.unsqueeze(5)
        vec2 = vec.unsqueeze(4)
        ...
        for j in range(8):
            ...
            for k in range(8):
                dy_k, dx_k = offsets[k]
                s = dx_j * dy_k - dx_k * dy_j
                row.append(1.0 if s > 0 else (-1.0 if s < 0 else 0.0))
            signs.append(row)
        
        sign_matrix = torch.tensor(signs, device=points_3d.device).view(1, 1, 1, 1, 8, 8)
        sum_n = (cross * sign_matrix).sum(dim=[4, 5])
        
        norm = torch.norm(sum_n, dim=1, keepdim=True)
        normals = sum_n / norm.clamp(min=1e-12)
